        ...
        return (value) => {
          ...
          if (s.length > length) {
            ...
            return (
              s.substring(0, x) +
              "..." +
              s.substring(s.length - (length - 3 - x))
            );
          }
          return s;
        };
...
let format: {
  cacheIdentifier?: number;
  cache: Record<string, Formatter>;
} = {
  cache: {},
};
...
export class Format {
  ...
  static register(format, formatter: Formatter) {
...
export function resolveMinMaxFractionDigits(
  ...
  if (isNumber(minimumFractionDigits)) {
    if (isUndefined(maximumFractionDigits))
      ...
      isNumber(maximumFractionDigits) &&
      maximumFractionDigits < minimumFractionDigits
    )
      maximumFractionDigits = minimumFractionDigits;
