...
import { StringProp, StructuredProp } from "../core";
import { RenderingContext } from "../ui/RenderingContext";
import { Instance } from "../ui/Instance";

let isDataAttribute = (attr) =>
...
export interface HtmlElementConfig extends ContainerConfig {
  html?: StringProp;
  text?: StringProp;
  innerText?: StringProp;
  innerHtml?: StringProp;
  extraProps?: StructuredProp;
...
export class HtmlElement extends Container {
  ...
  constructor(config: HtmlElementConfig) {
    super(config);

    if (isUndefined(this.jsxAttributes) && config)
      this.jsxAttributes = Object.keys(config).filter(
        this.isValidHtmlAttribute.bind(this),
      );
  }
    ...
    if (this.jsxAttributes) {
      this.jsxAttributes.forEach((attr) => {
        ...
        if ((name = isDataAttribute(attr))) {
          ...
          (name = this.isValidHtmlAttribute(attr)) &&
  ...
  render(context: RenderingContext, instance: Instance, key: string) {
    ...
    if (this.events && !instance.events) {
      ...
      for (let eventName in this.events) {
        instance.events[eventName] = (e) =>
          instance.invoke(eventName, e, instance);
      }
    }
