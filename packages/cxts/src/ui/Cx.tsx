...
export class Cx extends VDOM.Component {
  ...
  render() {
    if (
      this.props.deferredUntilIdle &&
      this.state.deferToken < this.deferCounter
    )
      return null;
...
class CxContext extends VDOM.Component {
  ...
  UNSAFE_componentWillReceiveProps(props) {
    ...
    do {
      ...
      this.props.flags.dirty &&
      ++count <= 3 &&
      Widget.optimizePrepare &&
      now() - this.timings.start < 8
    );
    ...
    if (visible) {
      ...
      this.content = null;
      this.timings.afterExplore =
        this.timings.afterPrepare =
        this.timings.afterRender =
          now();
    }
