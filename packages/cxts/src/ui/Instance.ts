...
export class Instance {
  ...
  cacheList: Record<string, any> | null;
  store: View;
  dataSelector: any;
  explored: boolean;
  prepared: boolean;
  needsExploreCleanup?: boolean;
  visible: boolean;
  detached: boolean;
  childStateDirty: boolean;
  state: any;
  rawData: any;
  data: any;
  initialized: boolean;
  ...
  outerLayout?: Instance;
  instanceCache?: InstanceCache | null;
  renderList: any;
  controller: Controller;
  parentOptions: any;
  destroyTracked: boolean;
  contentPlaceholder: Instance;
  helpers: { [key: string]: Instance };
  components: { [key: string]: Instance };
  vdom: any;
  contentVDOM: any;
  ...
  constructor(widget: Widget, key: string, parent: Instance, store: View) {
    this.widget = widget;
    this.key = key;
  ...
  render(context) {
    ...
    if (this.cacheList)
      for (let key in this.cacheList) this.cached[key] = this.cacheList[key];
  ...
  set(prop, value, options: { internal?: boolean; immediate?: boolean } = {}) {
...
export class InstanceCache {
  ...
  parent: Instance;
  marked: Record<string, Instance>;
  keyPrefix: string;
  monitored: Record<string, Instance> | null;
  ...
  getChild(widget: Widget, store: View, key?: string | null) {
    let k =
      this.keyPrefix + (key != null ? key : widget.vdomKey || widget.widgetId);
    ...
    if (
      ...
      instance = new Instance(widget, k, this.parent, this.parent.store);
