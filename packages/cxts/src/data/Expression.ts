...
let expCache = {},
  ...
  helperNames: string[] = [],
  helperValues: unknown[] = [],
  expFatArrows: (code: string) => string;

function getExpr(expr): Selector {
      ...
      if (!getter) {
        ...
        if (colonIndex != -1) {
          ...
          if (nullSeparatorIndex != -1) {
            format = Format.parse(
              bindingWithFormat.substring(nullSeparatorIndex)
            );
...
export function expression(str): Selector {
      ...
      case "}":
        if (termStart >= 0) {
          if (--curlyBrackets == 0) {
            ...
            if (
              percentExpression ||
              (binding[0] == "[" && binding[binding.length - 1] == "]")
            ) {
              argName = `expr${++subExpr}`;
              args[argName] = expression(
                percentExpression
                  ? binding
                  : binding.substring(1, binding.length - 1)
              );
  ...
  try {
    let compute = new Function(
      ...formats.map((f, i) => "fmt" + i),
      ...keys,
      ...helperNames,
      body
    ).bind(Format, ...formats, ...helperValues);
    let selector = computable(...keys.map((k) => args[k]), compute);
    expCache[str] = selector;
    return selector;
  } catch (err) {
    throw new Error(
      `Failed to parse expression: '${str}'. Error: ${err.message}`
    );
  }
}
...
export const Expression = {
  ...
  registerHelper: function (name: string, helper: unknown) {
    helpers[name] = helper;
